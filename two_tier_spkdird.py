...
from datasets.dataset import audio_train_feed_epoch as train_feeder
from datasets.dataset import audio_valid_feed_epoch as valid_feeder
from datasets.dataset import audio_test_feed_epoch  as test_feeder
...
def load_data(data_feeder):
    ...
    """
    return data_feeder(AUDIO_NUMPY_PATH, 
                       BATCH_SIZE,
                       SEQ_LEN,
                       OVERLAP,
                       Q_LEVELS,
...
def frame_level_rnn(input_sequences, spkids, h0, reset):
    """
    ...
    #####change the ori feature input into one-hot representation
    #frames=lib.ops.T_one_hot(frames, 256) # 256 integers, shape should be (batch size, n_frame, FRAME_SIZE, 256)
    #frames = frames.reshape((input_sequences.shape[0], input_sequences.shape[1] // FRAME_SIZE, FRAME_SIZE*256))
    
    n_frame=input_sequences.shape[1] // FRAME_SIZE
    #spkids=numpy.tile(spkids, (input_sequences.shape[1] // FRAME_SIZE,1)) # copy
    spkids=T.tile(spkids, (n_frame, 1)) # # repeat 3x in the first dimension, 1x in the second
    spkids=spkids.dimshuffle(1,0)
    #spkids=spkids.T # shape = (N, (input_sequences.shape[1] // FRAME_SIZE))
    spkids = lib.ops.T_one_hot(spkids, 15) # 2^4=16, shape now is : (128, frame_num, 15), 15 scene classes
    
    print spkids.shape
    frames = T.concatenate([ # shape now is : (128, frame_num, FRAME_SIZE+15) ???
            frames,
            spkids
        ], axis=2)    
...
def sample_level_predictor(frame_level_outputs, prev_samples, spkids):
    """
...
spkids      = T.ivector('spkids') ########################### size=128 for one batch
h0        = T.tensor3('h0')
...
frame_level_outputs, new_h0 =\
    frame_level_rnn(input_sequences, spkids, h0, reset)
...
train_fn = theano.function(
    [sequences, spkids, h0, reset, mask],
    [cost, new_h0],
    updates=updates,
...
frame_level_generate_fn = theano.function(
    [sequences, spkids, h0, reset],
    frame_level_rnn(sequences, spkids, h0, reset),
    on_unused_input='warn'
)
...
sample_level_generate_fn = theano.function(
    [frame_level_outputs, prev_samples, spkids],
...
def monitor(data_feeder):
    ...
    for _seqs, _sids, _reset, _mask in _data_feeder:
        _cost, _h0 = test_fn(_seqs, _sids, _h0, _reset, _mask)
        _costs.append(_cost)
...
while True:
    ...
    seqs, sids, reset, mask = mini_batch
